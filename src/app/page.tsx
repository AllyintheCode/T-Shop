...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        // Products
        const prodRes = await fetch("https://t-shop-backend-chi.vercel.app/api/products");
        const prodData = await prodRes.json();
        setProducts(prodData);

        // Categories
        const catRes = await fetch("https://t-shop-backend-chi.vercel.app/api/categories");
        ...
        if (token) {
          const wishRes = await fetch("https://t-shop-backend-chi.vercel.app/api/users/wishlist", {
  ...
  const toggleWishlist = async (product: Product) => {
    ...
    try {
      const res = await fetch(`https://t-shop-backend-chi.vercel.app/api/users/wishlist/${product._id}`, {
        method: "POST",
        headers: { token: `Bearer ${token}` }
      });
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("https://t-shop-backend-chi.vercel.app/api/orders", {
        method: "POST",
        headers: { 
